...
create table _cart (buyer_id integer not null, id integer not null, payment_method_id integer, shipping_address_id integer, created_time datetime(6), primary key (id)) engine=InnoDB;
...
create table _order_item (amount integer not null, id integer not null, order_id integer not null, price decimal(38,2), product_id integer not null, created_time datetime(6), primary key (id)) engine=InnoDB;
...
create table _transparent_policy (id integer not null, created_time datetime(6), name enum ('AVAILABLE_PRODUCT_ORDERS','AVAILABLE_PRODUCT_ORDERS_BUYER','AVAILABLE_PRODUCT_ORDERS_DATE','AVAILABLE_PRODUCT_ORDERS_PRICE'), type varchar(255), primary key (id)) engine=InnoDB;
...
create table authentication_transparent_policy (authentication_id integer not null, transparent_policy_id integer not null) engine=InnoDB;
...
alter table _order add constraint FK435eu2rx3g2y30l5rhj8u96xl foreign key (buyer_id) references _buyers (id) on delete cascade;
...
alter table authentication_role add constraint FK3xq58x5a2dmqte9n69erwso56 foreign key (role_id) references _roles (id) on delete cascade;
alter table authentication_role add constraint FKlmygagyqgxwcl36jw42ksknjj foreign key (authentication_id) references _authentication (id) on delete cascade;
alter table authentication_transparent_policy add constraint FKfo9i6u441yv3577rva5ik1m7e foreign key (transparent_policy_id) references _transparent_policy (id) on delete cascade;
